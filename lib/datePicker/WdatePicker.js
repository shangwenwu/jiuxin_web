...
(function() {
	...
	A = $.$dpPath || J();
	if ($.$wdate) K(A.replace('lib/datePicker/', '') + 'css/lib/css/datePicker.css');
	V = Y;
	if ($.$crossFrame) {
		try {
		...
		function E(K, J) {
			...
			function C() {
				...
				for (var C = 0; C < D.length; C++)
					if (D[C].name == J.skin) $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + A.replace('lib/datePicker/', '') + "css/lib/css/datePicker.css\" charset=\"" + D[C].charset + "\"/>");
