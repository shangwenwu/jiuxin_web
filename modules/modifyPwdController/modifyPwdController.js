var ModifyPwdController = function(){
	var t = this;
	t.el = $('<div id="modifyPwdModule" class="modifyPwd_module">'+
				'<p class="statusTitle">修改密码</p>'+
				'<p class="f18 pad100 mt30">修改登录密码</p>'+
				'<div class="statusBox1">'+
					'<label for="oldPassword" class="mt30"><span>原密码</span><input type="password" id="oldPassword"  placeholder="请输入原密码" /><p class="red"></p></label>'+
					'<label for="password" class="mt20"><span>设置密码</span><input type="password" id="password"  placeholder="6-20位数字和字母及特殊符号组成" /><p class="red"></p></label>'+
					'<label for="againPassword" class="mt20"><span>再次输入</span><input type="password" id="againPassword"  placeholder="请再次输入密码" /><p class="red"></p></label>'+
				...
				'</div>'+
				'<p class="f18 pad100 mt80">修改支付密码</p>'+
...
ModifyPwdController.prototype = {
	...
	events : function(){
	    ...
	    t.el.delegate('#submit', 'click', function(e){ 
	    	if(J.Common.regFun(t.regEleInfo.oldPassword) && J.Common.regFun(t.regEleInfo.password) && J.Common.regFun(t.regEleInfo.againPassword) && t.captchaResult){
